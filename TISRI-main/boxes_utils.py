...
def get_visual_label(rois_boxes,GT_boxes):
    ...
    target_label=np.zeros((num_GT_boxes,num_roi_boxes,4)) #[1,100] #if IoU >= 0.5,= GT offset; =0
    ...
    gt_target_boxes = GT_boxes[gt_assignment, :4]  #[100] the "true" value of each roi

    for i,ious in enumerate(IoUs): # multiple GT_boxes
        if ious.max()>=0.5:
...
def get_attention_label(rois_boxes, GT_boxes):  # --
    ...
    tensor_roi = torch.from_numpy(rois_boxes)  # torch.from_numpy() converts array to tensor, and the two share memory
    tensor_GT = torch.from_numpy(GT_boxes)
    ...
    for i, ious in enumerate(IoUs):  # multiple GT_boxes
        if ious.max() >= 0.5:
            pos_idx = np.where(ious >= 0.5)  # np.where(condition) the function returns the coordinates (index) of the element that satisfies the condition
...
def compute_targets(ex_rois, gt_rois, query_label):
    ...
    targets = bbox_transform(ex_rois, gt_rois)  # calculate the GT offset that has the largest IOU with ex_rois
...
def clip_boxes(boxes, im_shape):
    ...
    # this is to ensure that the detection of the image edges results in a box that does not exceed the image size. mainly x2 and y2. a safeguard mechanism that generally doesn't work.
    """
